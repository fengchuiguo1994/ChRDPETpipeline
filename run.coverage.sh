...
computeMatrix reference-point --referencePoint TSS -S $tmpdir/$OUTPUT_PREFIX.DNA.1x.bw --regionsFileName $BED_file -a $EXT_LEN_up -b $EXT_LEN_down --skipZeros --missingDataAsZero --outFileName $tmpdir/$OUTPUT_PREFIX.DNA.TSS.gz --numberOfProcessors $NTHREADS; plotProfile -m $tmpdir/$OUTPUT_PREFIX.DNA.TSS.gz -out $tmpdir/$OUTPUT_PREFIX.DNA.TSS.pdf
computeMatrix scale-regions -S $tmpdir/$OUTPUT_PREFIX.RNA.1x.bw -R $BED_file -a $EXT_LEN_up -b $EXT_LEN_down -m $GENE_LEN --skipZeros --outFileName $tmpdir/$OUTPUT_PREFIX.RNA.gene.gz --numberOfProcessors $NTHREADS --skipZeros --missingDataAsZero ; plotProfile -m $tmpdir/$OUTPUT_PREFIX.RNA.gene.gz -out $tmpdir/$OUTPUT_PREFIX.RNA.gene.pdf
computeMatrix scale-regions -S $tmpdir/$OUTPUT_PREFIX.RNA.fwd.bw $tmpdir/$OUTPUT_PREFIX.RNA.rev.bw -R $BED_file -a $EXT_LEN_up -b $EXT_LEN_down -m $GENE_LEN --outFileName $tmpdir/$OUTPUT_PREFIX.RNA.sense.richheatmap.gz --outFileNameMatrix $tmpdir/$OUTPUT_PREFIX.RNA.sense.richheatmap.matrix --numberOfProcessors $NTHREADS --skipZeros --missingDataAsZero; python $PROGRAM_DIRECTORY/sensecg.py $tmpdir/$OUTPUT_PREFIX.RNA.sense.richheatmap.gz | gzip > $tmpdir/$OUTPUT_PREFIX.RNA.sense.richheatmap.cg.gz; plotProfile -m $tmpdir/$OUTPUT_PREFIX.RNA.sense.richheatmap.cg.gz -out $tmpdir/$OUTPUT_PREFIX.RNA.sense.richheatmap.cg.pdf --perGroup
